...
Rectangle.prototype.erase = function() {
	...
	ctx1.strokeStyle = Tetris.BOARD_COLOR;
	ctx1.rect(this.px, this.py, this.width, this.height);
	ctx1.stroke();
	ctx1.fillStyle = Tetris.BOARD_COLOR;
	ctx1.fill()
}
...
Board.prototype.game_over = function() {
	...
	over_background.onload = function(){
		ctx1.drawImage(over_background, 0, 0);
	}
}
...
Tetris.prototype.key_pressed = function(e) {
	...
	switch(e.keyCode) {
		...
		case 77:
			// M key pressed
			if ((!this.game_paused && !this.game_over) || !game2.game_over) {
    ...
    if (this.current_shape.can_move(this.board, x, y)) {
        ...
        if (direction == "Down") {
            ...
            if (!this.board.draw_shape(this.current_shape)) {
				...
				this.game_over = true;
				if (this.game_over && game2.game_over) { // Only stopping the music if both players have lost.
...
Tetris.prototype.pause_resume_game = function() {
	...
	if (this.game_paused) {
		...
		document.getElementById('pause1').setAttribute('style','display: initial');
		clearInterval(this.reloj);
...
Tetris.prototype.show_next_shape = function() {
	document.getElementById("shape1").src = "images/" + this.next_shape.getName() + ".png";
};

Tetris.prototype.show_level = function() {
	document.getElementById('level1').innerHTML = this.level;
	this.animate_shape();
};
...
Tetris.prototype.update_record = function() {
	if (record < this.score) {
		...
		document.getElementById('record1').innerHTML = this.score;
	}
};
...
Tetris.prototype.pause_resume_music = function() {
	if (background_audio.paused) {
		if (!this.game_muted) {
			...
			document.getElementById('sound1').setAttribute('style','display: initial');
			document.getElementById('mute2').setAttribute('style','display: none');
			document.getElementById('sound2').setAttribute('style','display: initial');
			background_audio.play();
		}
	} else {
		document.getElementById('mute1').setAttribute('style','display: initial');
		document.getElementById('sound1').setAttribute('style','display: none');
		document.getElementById('mute2').setAttribute('style','display: initial');
		document.getElementById('sound2').setAttribute('style','display: none');
		background_audio.pause();
	}
}